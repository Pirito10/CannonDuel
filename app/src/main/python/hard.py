...
class Agent:
    def __init__(self, q_table_move_file='q_table_move.npy',
                 q_table_shoot_file='q_table_shoot.npy'):
    ...
    def shoot(self, wind_direction, available_cells):
        ...
        if random.uniform(0, 1) < self.epsilon:
            return random.choice(available_cells)
        q_values = [self.q_table_shoot[cell[0], cell[1], wind_direction] for cell in
                    available_cells]
        return available_cells[np.argmax(q_values)]
    ...
    def update_move_q_table(self, current_position, fuel, next_position, reward):
        ...
        self.q_table_move[state[0], state[1], fuel] += self.alpha * (
                reward + self.gamma * best_next_q - self.q_table_move[state[0], state[1], fuel])
    ...
    def update_shoot_q_table(self, wind_direction, target_position, reward):
        state = (wind_direction[0], wind_direction[1])
        best_next_q = np.max(
            self.q_table_shoot[target_position[0], target_position[1], wind_direction])
        self.q_table_shoot[state[0], state[1], wind_direction] += self.alpha * (
                reward + self.gamma * best_next_q - self.q_table_shoot[
            state[0], state[1], wind_direction])
        self.save_q_table(self.q_table_shoot, self.q_table_shoot_file)

    def handle_turn(action_type, current_state, params, reward=None):
        if action_type == "move":
            available_cells, fuel = params
            return agent.move(available_cells, fuel, current_state)
        elif action_type == "shoot":
            available_cells, wind_direction = params
            return agent.shoot(wind_direction, available_cells)
        elif action_type == "update_move":
            next_position = params[0]
            agent.update_move_q_table(current_state, params[1], next_position, reward)
        elif action_type == "update_shoot":
            target_position = params[0]
            agent.update_shoot_q_table(current_state, target_position, reward)
